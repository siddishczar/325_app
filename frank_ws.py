...
url = 'https://umassdining.com/menu/franklin-grab-n-go'
response = requests.get(url)
if response.status_code == 200:
    soup = BeautifulSoup(response.text, 'html.parser')
    #soup1 = soup.find('div', class_='breakfast_fp', id='content_text')
    soup2 = soup.find('div', class_='lunch_fp', id='content_text')
    ...
    try:
            ...
            cursor = cnx.cursor()
            cursor.execute("DELETE FROM frank_dc")
            add_drinks = ("INSERT INTO frank_dc (khana, type) VALUES (%s, %s)")
            #for i in range(len(dish_names)):
            #    sodas = dish_names[i]
            #    data_drink = (sodas, 'breakfast')
            #    cursor.execute(add_drinks, data_drink)
            add_drink = ("INSERT INTO frank_dc (khana, type) VALUES (%s, %s)")
