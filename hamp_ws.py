## This scraper is for Frank DC.
import requests
from bs4 import BeautifulSoup
import mysql.connector
from mysql.connector import errorcode

# This is for Frank DC
url = 'https://umassdining.com/menu/hampshire-grab-n-go'
response = requests.get(url)
if response.status_code == 200:
    soup = BeautifulSoup(response.text, 'html.parser')
    #soup1 = soup.find('div', class_='breakfast_fp', id='content_text')
    soup2 = soup.find('div', class_='breakfast_fp', id='content_text')
    #dish_name_elements = soup1.find_all(attrs={"data-dish-name": True})
    #dish_names = [element['data-dish-name'] for element in dish_name_elements]
    dish_name_element = soup2.find_all(attrs={"data-dish-name": True})
    ...
    try:
            ...
            cursor = cnx.cursor()
            cursor.execute("DELETE FROM hamp_dc")
            add_drinks = ("INSERT INTO hamp_dc (khana, type) VALUES (%s, %s)")
            #for i in range(len(dish_names)):
            #    sodas = dish_names[i]
            #    data_drink = (sodas, 'breakfast')
            #    cursor.execute(add_drinks, data_drink)
            add_drink = ("INSERT INTO hamp_dc (khana, type) VALUES (%s, %s)")
            for j in range(len(dish_name)):
                soda = dish_name[j]
                data_drinks = (soda, 'breakfast')
