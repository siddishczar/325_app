...
url = 'https://umassdining.com/menu/berkshire-grab-n-go-menu'
...
if response.status_code == 200:
    ...
    try:
            ...
            cursor = cnx.cursor()
            cursor.execute("DELETE FROM berk_dc")
            add_drinks = ("INSERT INTO berk_dc (khana, type) VALUES (%s, %s)")
            for i in range(len(dish_names)):
                sodas = dish_names[i]
                data_drink = (sodas, 'breakfast')
                cursor.execute(add_drinks, data_drink)
            add_drink = ("INSERT INTO berk_dc (khana, type) VALUES (%s, %s)")
